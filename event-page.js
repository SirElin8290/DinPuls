...
function renderEventSource(source,health){const status=health?.status==="ok"?`Kontrollerad · ${Number(health.events)||0} maskinläsbara evenemang` : health?.status==="error"?"Källan svarade inte vid senaste kontrollen":"Kontrollstatus saknas";return `<a class="event-source" href="${escapeEvent(source.url)}" target="_blank" rel="noopener noreferrer"><span class="portal-card-icon event"><i data-lucide="${escapeEvent(source.icon||"calendar-days")}"></i></span><span><strong>${escapeEvent(source.name)}</strong><small>${escapeEvent(source.type)} · ${escapeEvent(status)}</small></span><i data-lucide="external-link"></i></a>`}
