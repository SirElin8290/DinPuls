...
async function openFoundationCountersign(contract, dialog) {
  ...
  async function refreshBilling() {
    ...
    try {
      const [data, spiris] = await Promise.all([api("/portal/admin/billing/orders"), api("/portal/admin/spiris/status")]);
      ...
      const connection = `<article class="panel spiris-connection"><div><span class="badge ${spiris.connected ? "free" : ""}">${spiris.connected ? "Sandbox ansluten" : "Inte ansluten"}</span><h2>Spiris sandbox</h2><p>${spiris.connected ? `OAuth är anslutet sedan ${escapeHtml(new Date(spiris.connectedAt).toLocaleString("sv-SE"))}.` : "Anslut DinPuls med Spiris OAuth. Fakturering förblir avstängd."}</p><p class="muted small">${escapeHtml(spiris.redirectUri)} · ${spiris.scopes.map(escapeHtml).join(" ")}</p></div><button id="connectSpiris" class="primary" type="button" ${!spiris.configured ? "disabled" : ""}>${spiris.connected ? "Anslut igen" : "Anslut Spiris"}</button></article>`;
      ...
      target.innerHTML = connection + orders;
      $("#connectSpiris").onclick = async () => { try { const result = await api("/portal/admin/spiris/connect", { method: "POST", body: "{}" }); location.assign(result.authorizationUrl); } catch (error) { alert(error.message); } };
