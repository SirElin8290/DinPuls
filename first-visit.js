...
(() => {
  ...
  const enhance = () => {
    ...
    card.insertBefore(label, combo.parentNode ? combo : note);
    label.after(combo);
    ...
    const filter = () => {
      ...
      options.querySelectorAll("[data-first-municipality]").forEach(button => {
        button.hidden = !button.dataset.firstMunicipality.toLocaleLowerCase("sv-SE").includes(query);
      });
    };
    ...
    proceed.addEventListener("click", () => selected?.click());
