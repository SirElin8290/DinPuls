...
def fetch(url):
    ...
    except HTTPError as error: raise RuntimeError(f"HTTP {error.code}") from None
    except URLError as error: raise RuntimeError(str(error.reason)) from None
    except TimeoutError: raise RuntimeError("timeout") from None
...
def useful_dish(line):
    lowered=line.lower()
    if len(line)<5 or len(line)>240 or any(lowered.startswith(marker) for marker in STOP_MARKERS) or lowered in NON_DISH_LINES: return False
...
def build_output(config,now,fetcher=fetch):
    ...
    for municipality,sources in config.get("municipalities",{}).items():
        ...
        for source in sources:
            ...
            if source.get("seasonal"): item["seasonal"]=True
...
def main():
    config=json.loads(SOURCES.read_text(encoding="utf-8")); now=datetime.now(TIMEZONE); output=build_output(config,now)
