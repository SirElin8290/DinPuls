...
try {
  ...
  assert.ok(purchases.purchases.every(item => item.foundation_contract_id === contractId && item.billing_status === "awaiting_approval"));
  ...
  const billing = await read("/portal/admin/billing/orders", "GET", null, admin.token);
  ...
  assert.equal(approvedAgain.idempotent, true, "Retry får inte skapa ett nytt godkännande");
  const approvalRows = await db.prepare("SELECT COUNT(*) count FROM billing_approvals WHERE order_id=?").bind(first.orderId).first();
  assert.equal(Number(approvalRows.count), 1);
  const approvedPurchases = await db.prepare("SELECT billing_status FROM self_service_purchases WHERE order_id=?").bind(first.orderId).all();
  assert.ok(approvedPurchases.results.every(item => item.billing_status === "approved_for_invoice"));
  await read(`/portal/admin/billing/orders/${first.orderId}/spiris-draft`, "POST", {}, admin.token, 503);
