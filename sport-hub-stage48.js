(() => {
  ...
  function renderMatches(){
    ...
    return `${ad(1)}${seasonCards()}${visible.join("")}${visible.length<6?ads(...Array.from({length:6-visible.length},(_,i)=>visible.length+i+2)):""}${ad(8)}`;
  }
  function renderTables(){
    ...
    const sections=activeSeries.map((item,index)=>`<section class="sport-hub-section sport-hub-series-card"><header class="sport-hub-section-head"><div><span class="section-kicker">${esc(item.sport)} · ${esc(item.team)}</span><h2>${esc(item.competition)}</h2><p>${esc(item.statusLabel)}. Tabellen visas hos ansvarigt förbund eller förening så att placering och poäng alltid är aktuella.</p></div><a class="sport-hub-source" href="${esc(item.sourceUrl)}" target="_blank" rel="noopener noreferrer">Öppna officiell tabell ↗</a></header></section>${index<6?ad(index+2):""}`);
